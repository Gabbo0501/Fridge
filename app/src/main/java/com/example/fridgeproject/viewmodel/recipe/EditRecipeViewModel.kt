...
import com.example.fridgeproject.navigation.EditRecipeRoute
...
class EditRecipeViewModel (
    ...
    private val route = savedStateHandle.toRoute<EditRecipeRoute>()
    ...
    fun loadRecipe(id: String) {
        viewModelScope.launch {
            ...
            if (recipe != null) {
                ...
                _uiState.update {
                    it.copy(errors = it.errors.copy(globalError = "Recipe Details Not Found"))
                }
            }
    ...
    fun saveRecipe() {
        ...
        val newErrors = EditRecipeErrors(
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                val recipeBeforeEdit = originalRecipe ?: recipeRepository.getRecipeById(recipeId).first()

                if (recipeBeforeEdit == null) {
                    _uiState.update {
                        it.copy(
                            isSaving = false,
                ...
                val recipeToSave = recipeBeforeEdit.copy(
                    title = state.title,
                    image = state.image,
                ...
                _uiState.update {
                    it.copy(
                        ...
                        pendingStepImageInputs = emptyMap()
                    )
                }
                _events.send(RecipeEvent.Updated)
            } catch (e: Exception) {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                _uiState.update { currentState ->
                    val alreadySelected = currentState.ingredients.map { ingredientQuantity ->
                        ingredientQuantity.ingredient
                    }

                    val filteredIngredients = ingredientsFromDb.filterNot { ingredient ->
                        ingredient in alreadySelected
                    }

                    currentState.copy(selectableIngredients = filteredIngredients, isLoading = false)
                }
            } catch (e: Exception) {
                _uiState.update { it.copy(isLoading = false, errors = it.errors.copy(globalError = e.message)) }
            }
        }
    }
    ...
    fun onReset() = _uiState.update { EditRecipeUiState() }
    ...
    fun updateDishType(dishType: DishType) = _uiState.update {
        it.copy(dishType = dishType)
    }

    fun updateCostRange(costRange: CostRange) = _uiState.update {
        it.copy(costRange = costRange)
    }

    fun updateDifficulty(difficulty: Difficulty) = _uiState.update {
        it.copy(difficulty = difficulty)
    }

    fun updatePrepTime(prepTime: Long) =
        _uiState.update {
            it.copy(
                preparationTimeSec = prepTime,
                errors = it.errors.copy(prepTimeError = null)
            )
        }
    ...
    fun toggleDiet(diet: Diet) {
        _uiState.update {
            val newList =
                if (it.suitableDiets.contains(diet)) it.suitableDiets - diet else it.suitableDiets + diet
        ...
        _uiState.update { state ->
            state.copy(
                newIngredient = state.newIngredient.copy(
                    ingredient = state.newIngredient.ingredient.copy(category = category)
                )
            )
        }
    }
    ...
    fun updateStepDescription(description: String) {
        _uiState.update {
            ...
            it.copy(preparationSteps = newSteps)
        }
    }
    ...
    fun switchLeftStep(){
        _uiState.update {
            ...
            it.copy(currentEditedStepIndex = newStep)
        }
    }
